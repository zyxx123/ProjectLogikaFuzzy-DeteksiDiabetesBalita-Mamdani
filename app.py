...
def dashboard():
    ...
    if session.get('role') == 'admin':
        ...
        # Kader Dashboard: Statistik
        riwayat = dapatkan_riwayat_berdasarkan_pengguna(session['user_id'])
        pasien = dapatkan_pasien_berdasarkan_pengguna(session['user_id'])
        stats = {
            "total_pasien": len(pasien),
            "total": len(riwayat),
            "tinggi": sum(1 for r in riwayat if 'TINGGI' in r['label_risiko']),
            "sedang": sum(1 for r in riwayat if 'SEDANG' in r['label_risiko']),
            "rendah": sum(1 for r in riwayat if 'RENDAH' in r['label_risiko'] or 'NORMAL' in r['label_risiko'])
        }
        return render_template('kader.html', stats=stats)
...
def tambah_pasien_route():
    ...
    return redirect(url_for('data_pasien'))

@app.route('/hapus_pasien/<int:id_pasien>')
def hapus_pasien_route(id_pasien):
    if not session.get('logged_in'):
        return redirect(url_for('login'))

    if hapus_pasien(id_pasien):
        flash('Data pasien berhasil dihapus!', 'success')
    else:
        flash('Gagal menghapus pasien!', 'error')

    return redirect(url_for('data_pasien'))    

@app.route('/hapus_riwayat/<int:id_riwayat>')
def hapus_riwayat_route(id_riwayat):
    if not session.get('logged_in'):
        return redirect(url_for('login'))

    if hapus_riwayat(id_riwayat):
...
@app.route('/edit_pasien/<int:id_pasien>', methods=['GET', 'POST'])
def edit_pasien_route(id_pasien):
    if not session.get('logged_in'):
        return redirect(url_for('login'))

    pasien = dapatkan_pasien_by_id(id_pasien)
    ...
    if request.method == 'POST':
        ...
        if update_pasien(id_pasien, nama, jk, tgl_lahir, nama_ortu):
            flash('Data pasien berhasil diperbarui!', 'success')
        else:
            flash('Gagal memperbarui data pasien!', 'error')
...
def tambah_user_route():
    ...
    if tambah_pengguna(username, password, role):
...
def hapus_user_route(id_user):
    if not session.get('logged_in') or session.get('role') != 'admin':
        return redirect(url_for('login'))
        
    if session.get('user_id') == id_user:
        flash("Anda tidak dapat menghapus akun Anda sendiri saat sedang login!", "error")
        return redirect(url_for('data_user'))

    if hapus_pengguna(id_user):
        flash('Pengguna berhasil dihapus beserta data terkait!', 'success')
    else:
...
def edit_user_route(id_user):
    ...
    user = dapatkan_pengguna_by_id(id_user)
    if not user:
    ...
    if request.method == 'POST':
        username = request.form['username']
        role = request.form['role']
        password = request.form['password'] # optional
        
        # If password is empty, don't update it
        pwd_to_update = password if password.strip() != '' else None
        
        if update_pengguna(id_user, username, role, pwd_to_update):
...
def data_pasien():
    ...
    print(pasien)
    return render_template('data_pasien.html', pasien=pasien)
